...
Click "Rent" when you've found an instance that works for you
...
HTTPS Option: Want secure connections? Set ENABLE_HTTPS=true in the Environment Variables section of your Vast.ai account settings page. You'll need to install the Vast.ai certificate to avoid browser warnings.
...
PROVISIONING_MANIFEST (recommended) - URL or local path to a declarative YAML manifest. The provisioner handles install ordering, retries, idempotency, and logging for you.
...
PROVISIONING_MANIFEST		URL or path to a declarative YAML provisioning manifest (recommended)
...
DISABLE_VGL	false	Set true to disable VirtualGL GPU acceleration for the desktop (use if an app misbehaves under VGL)
...
If your workflow requires features that don't work in a container, try the Ubuntu Desktop (VM) template instead. The VM template provides a full virtual machine with complete isolation and no container restrictions.

Licenses
This template ships vendor application(s) under the following license(s):

Selkies-GStreamer — MPL-2.0 (upstream)
Apache Guacamole — Apache-2.0 (upstream)
Blender — GPL-2.0-or-later (upstream)
See /LICENSES.md in the image for license details and file locations.

Need More Help?
Selkies Project: GitHub Repository
Apache Guacamole: Official Documentation
Image Source & Features: GitHub Repository
Instance Portal Guide: Vast.ai Instance Portal Documentation
SSH Setup Guide: Vast.ai SSH Documentation
